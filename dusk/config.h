...
static       char dmenufont[]            = "IosevkaTree Nerd Font:style=SemiBold:pixelsize=14:antialias=true:autohint=true";
...
#define SHCMD(cmd) { .v = (const char*[]){ NULL, "/bin/zsh", "-c", cmd, NULL } }
...
static Key keys[] = {
	...
	{ KeyPress,   MODKEY,                             XK_Delete,       spawn,                  {.v = rofipower } }, // spawn dmenu for launching other programs
	...
	{ KeyPress,   MODKEY,                             XK_r,            layoutconvert,          {0} }, // flip between horizontal and vertical layout
	{ KeyPress,   MODKEY,                             XK_space,        setlayout,              {-1} }, // toggles between current and previous layout

	{ KeyPress,   MODKEY,                             XK_a,            focusstack,             {.i = +1 } }, // focus on the next client in the stack
	{ KeyPress,   MODKEY,                             XK_s,            focusstack,             {.i = +2 } }, // focus on the next client in the stack
	{ KeyPress,   MODKEY,                             XK_d,            spawn,                  {.v = termcmd } }, // spawn a terminal
	...
	{ KeyPress,   MODKEY,                             XK_x,            killclient,             {0} }, // close the currently focused window
	{ KeyPress,   MODKEY|Shift,                       XK_x,            restart,                {0} }, // restart dusk
	...
	{ KeyPress,   MODKEY,                             XK_Down,         setmfact,               {.f = -0.05} }, // decrease the size of the master area compared to the stack area(s)
	{ KeyPress,   MODKEY,                             XK_Up,           setmfact,               {.f = +0.05} }, // increase the size of the master area compared to the stack area(s)
	...
	{ KeyPress,   MODKEY|Ctrl,                        XK_m,            markall,                {0} }, // marks all clients on the selected workspace
	{ KeyPress,   MODKEY|Shift,                       XK_m,            markall,                {2} }, // marks all hidden clients on the selected workspace
	...
	{ KeyPress,   MODKEY,                             XK_comma,        viewwsdir,              {.i = -1 } }, // view the workspace on the immediate left of current workspace (on the current monitor)
	{ KeyPress,   MODKEY,                             XK_period,       viewwsdir,              {.i = +1 } }, // view the workspace on the immediate right of current workspace (on the current monitor)

	{ KeyPress,   MODKEY|Ctrl,                        XK_comma,        movewsdir,              {.i = -1 } }, // move client to workspace on the immediate left of current workspace (on the current monitor)
	{ KeyPress,   MODKEY|Ctrl,                        XK_period,       movewsdir,              {.i = +1 } }, // move client to workspace on the immediate right of current workspace (on the current monitor)
...
static Button buttons[] = {
	...
	{ ClkWorkspaceBar,           MODKEY|Ctrl,             Button1,        swapws,           {0} }, // swaps all clients on current workspace with that of the given workspace
