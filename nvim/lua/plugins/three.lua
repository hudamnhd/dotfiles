...
-- stylua: ignore start
      ...
      -- stylua: ignore end
       ...
       vim.keymap.set("n", "s<space>", three.open_project, { desc = "[F]ind [P]roject" })
  ...
  },
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<space><c-c>", '<CMD>lua require("Buffers").only()<CR>') -- BufCloseAllButCurrent
      vim.keymap.set("n", "<c-q>", '<CMD>lua require("Buffers").delete()<CR>')
      vim.keymap.set("n", "<space><c-q>", '<CMD>lua require("Buffers").clear()<CR>') -- BufCloseAll
    end,
  },
}
