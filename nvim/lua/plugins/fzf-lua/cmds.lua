...
_G.fzf_workdirs = function(opts)
  if not opts then opts = {} end

  -- workdirs.lua returns a table of workdirs
  local ok, dirs = pcall(function() return require("utils.workdirs").get() end)
  ...
  opts.actions = {
    ["default"] = function(selected)
      ...
      require("utils.other").set_cwd(newcwd)
    end
  }
...
_G.fzf_dirs = function(opts)
  ...
  fzf_lua.fzf_exec("fd . ~ --type d -H --follow --exclude .git --exclude .cache", opts)
end

-- map our provider to a user command ':Directories'
vim.cmd([[command! -nargs=* Directories lua _G.fzf_dirs()]])
vim.cmd([[command! -nargs=* Workdirs lua _G.fzf_workdirs()]])
