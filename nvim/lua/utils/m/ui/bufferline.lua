...
local bl = require("utils.m.buf")
local H = require("utils.m.ui.palette").hl
...
local function get()
  ...
  for i, bufnr in ipairs(bl.list) do
    ...
    bufs[i] = {
      ...
      readonly = (get_opt(bufnr, "readonly") or not get_opt(bufnr, "modifiable"))
...
local function render_bufs(width)
  ...
  for _, buf in ipairs(bufs) do
    ...
    -- if buf.alternate then
    --   name = H("L") .. " " .. name .. H("BS") .. "▕"
    -- end
    -- highlighting
    if buf.current then
      name = H("A") .. " " .. name .. H("AS") .. "▕"
    elseif buf.alternate then
        name = H("L") .. " " .. name .. H("BS") .. "▕"  -- Replace 'C' and 'CS' with your actual styling
    else
      name = H("B") .. " " .. name .. H("BS") .. "▕"
    end
