--Taken from: https://github.com/echasnovski/nvim/blob/4190cfee0b31863b7c525b4da0058058380c0db8/colors/neovim_colors.lua#L1
...
--stylua: ignore
local palette_dark = {
  grey1   = convert(l[1], c[1], h.grey),  -- NormalFloat
  grey2   = convert(l[2], c[1], h.grey),  -- Normal bg
  grey3   = convert(l[3], c[1], h.grey),  -- CursorLine
  grey4   = convert(l[4], c[1], h.grey),  -- Visual

  red     = convert(l[1], c[3], h.red),    -- DiffDelete
  yellow  = convert(l[1], c[3], h.yellow), -- Search
  green   = convert(l[1], c[3], h.green),  -- DiffAdd
  cyan    = convert(l[1], c[3], h.cyan),   -- DiffChange
  blue    = convert(l[1], c[3], h.blue),
  magenta = convert(l[1], c[3], h.magenta),
}

--stylua: ignore
local palette_light = {
  grey1   = convert(100 - l[1], c[1], h.grey),
  grey2   = convert(100 - l[2], c[1], h.grey),   -- Normal fg
  grey3   = convert(100 - l[3], c[1], h.grey),
  grey4   = convert(100 - l[4], c[1], h.grey),   -- Comment

  red     = convert(100 - l[3], c[2], h.red),     -- DiagnosticError
  yellow  = convert(100 - l[3], c[2], h.yellow),  -- DiagnosticWarn
  green   = convert(100 - l[3], c[2], h.green),   -- String,     DiagnosticOk
  cyan    = convert(100 - l[3], c[2], h.cyan),    -- Function,   DiagnosticInfo
  blue    = convert(100 - l[3], c[2], h.blue),    -- Identifier, DiagnosticHint
  magenta = convert(100 - l[3], c[2], h.magenta),
}
...
local enable_colorscheme = function()
  ...
  -- - Mini StatusLine
  hi('MiniStatuslineModeNormal',  { fg = bg.grey2, bg = fg.cyan })
  hi('MiniStatuslineModeInsert',  { fg = bg.grey2, bg = fg.blue })
  hi('MiniStatuslineModeVisual',  { fg = bg.grey2, bg = fg.magenta })
  hi('MiniStatuslineModeReplace', { fg = bg.grey2, bg = fg.yellow })
  hi('MiniStatuslineModeCommand', { fg = bg.grey2, bg = fg.red })

  -- - Mini TabLine
  hi('MiniTablineFill',            { link = 'Comment' })
  hi('MiniTablineHidden',          { link = 'Comment' })
  hi('MiniTablineCurrent',         { fg = bg.grey2, bg = fg.grey1 })
  hi('MiniTablineVisible',         { fg = bg.grey2, bg = fg.grey4 })
  hi('MiniTablineModifiedHidden',  { fg = bg.grey2, bg = fg.red })
  hi('MiniTablineTabpagesection',  { fg = bg.grey2, bg = fg.cyan })
  hi('MiniTablineModifiedCurrent', { fg = bg.grey2, bg = fg.blue })
  hi('MiniTablineModifiedVisible', { fg = bg.grey2, bg = fg.magenta })

  -- - tpope/vim-fugitive
  hi('fugitiveHash',              { link = 'Comment' })
  hi('fugitiveHeader',            { link = 'Comment' })
  hi('fugitiveHeading',           { fg = fg.cyan,   bold = true })
  hi('fugitiveHelpTag',           { fg = fg.yellow })
  hi('fugitiveSymbolicRef',       { fg = fg.blue })
  hi('fugitiveStagedModifier',    { fg = fg.green,  bold = true })
  hi('fugitiveUnstagedModifier',  { fg = fg.yellow, bold = true })
  hi('fugitiveUntrackedModifier', { fg = fg.red,    bold = true })
  hi('fugitiveStagedHeading',     { fg = fg.green,  bold = true })
  hi('fugitiveUnstagedHeading',   { fg = fg.yellow, bold = true })
  hi('fugitiveUntrackedHeading',  { fg = fg.red,    bold = true })
