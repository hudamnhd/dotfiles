...
local function ReloadFiles(opts)
...
function actions.switch_cwd_back()
  ...
  opts.cwd = opts.cwd or vim.uv.cwd()
  opts.query = fzf.config.__resume_data.last_query
  opts.cwd = vim.fn.fnamemodify(opts.cwd, ":h")
...
function actions.switch_cwd_next(dir)
  ...
  -- with previous cwd
  opts.fn_selected = nil
  print(opts.fn_selected)
  opts.cwd = dir or vim.uv.cwd()
  opts.query = fzf.config.__resume_data.last_query
  opts.cwd = vim.fn.fnamemodify(opts.cwd, ":h")
...
-- Definisikan fungsi yang menangani tindakan berdasarkan apakah path adalah direktori atau bukan
local function handle_path_action(selected, opts)
...
fzf.setup({
  ...
  defaults = {
    headers = { "actions" },
    actions = {
      ["ctrl-space"] = actions.switch_provider,
    },
  },
  args = {
    files_only = false,
  ...
  files = {
    actions = {
      ...
      --   handle_path_action(selected, opts)
      -- end,
    ...
    --       return string.format("tree -C %s | head -200", file.path)
    --     else
    --       return string.format("bat --color=always --style=numbers,changes %s", file.path)
    --     end
    --   end,
    -- },
    fzf_opts = {
