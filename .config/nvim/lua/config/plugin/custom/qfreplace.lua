...
function M.start(cmd) M.open_replace_window(cmd == nil or cmd == '' and 'split' or cmd) end

function M.open_replace_window(cmd)
  if vim.api.nvim_buf_is_loaded(buf_nr) then
    ...
    vim.api.nvim_create_autocmd('BufWriteCmd', {
      buffer = buf,
      callback = function() M.do_replace() end,
      nested = true,
    })
    ...
    local current_lines = {}
    for _, e in ipairs(M.get_effectual_lines(qflist)) do
      local text = M.chomp(e.text)
...
function M.do_replace()
  local qf = M.get_effectual_lines(vim.b.qfreplace_orig_qflist)
  ...
  for i, e in ipairs(qf) do
    ...
    if e.text ~= new_text then
      ...
      if current_line ~= M.chomp(e.text) then
...
function M.get_effectual_lines(qflist) return vim.tbl_filter(M.is_effectual_line, vim.deepcopy(qflist)) end

function M.is_effectual_line(qf) return qf.bufnr ~= nil and qf.bufnr ~= 0 end

function M.chomp(str) return string.gsub(str, '\r?\n?$', '') end

vim.api.nvim_create_user_command(
  'QfReplace',
  function(opts) M.start(opts.args) end,
  { nargs = '?', complete = 'command' }
)

function M.replace_by_pattern()
...
vim.api.nvim_create_user_command('QfReplacePattern', function() M.replace_by_pattern() end, {})
